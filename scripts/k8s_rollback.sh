...
if [[ -n "$TARGET_SLOT" ]]; then
  case "$TARGET_SLOT" in
    blue|green) target_slot="$TARGET_SLOT" ;;
    *) fail "TARGET_SLOT must be blue or green (got ${TARGET_SLOT})" ;;
  esac
else
...
log "rollback: namespace=${NAMESPACE} service=${ACTIVE_SERVICE} current=${current_slot} target=${target_slot}"

if [[ "$current_slot" == "$target_slot" ]]; then
  log "already on slot ${target_slot}; nothing to rollback"
  exit 0
fi
...
log "rollback succeeded: ${current_slot} -> ${target_slot}"
exit 0
